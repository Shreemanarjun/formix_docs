...
Future<FlutterApp> _flutterApp = Future(() {
  ...
  flutter!.loader!.didCreateEngineInitializer =
  ...
  ui_web.bootstrapEngine(
    runApp: () {
      flt.runWidget(
        MultiViewApp(
          viewBuilder: (viewId) => _viewWidgets[viewId] ?? _nextViewWidget,
        ),
      );
    },
  );
...
/// Widget to use for the next view being added.
/// Set just before [app.addView] so [MultiViewApp.viewBuilder] can access
/// it synchronously during the [didChangeMetrics] callback that fires inside
/// [app.addView] — before [_viewWidgets[id] = widget] on the next line.
flt.Widget? _nextViewWidget;
...
Future<int> addView(
  ...
  // Pre-register so viewBuilder can find it during the synchronous
  // didChangeMetrics callback triggered inside app.addView().
  _nextViewWidget = widget;
  final id = app.addView(
    ViewOptions(hostElement: target, viewConstraints: constraints),
  );
  _viewWidgets[id] = widget;
  _nextViewWidget = null;
  return id;
}
...
extension type EngineInitializer._(JSObject _) {
  ...
  external JSPromise<AppRunner> initializeEngine(
    InitializeEngineOptions options,
  );
}
